...
int main(int argc, const char * argv[]) {
	...
	int     CNT ;   //  steps of each photon
	int     row;
	
    // YL DETECTOR
	int     detflag;     //  1: photons arrive at detector
	float	circle;		
	float 	height;			
	int		Nphotons_Det;	
	float	effectivity;			
	int		detYL;      // 0: all photons; 1: only photons at Det
	
	/* mcxyz bin variables */
	float	dx, dy, dz;     /* bin size [cm] */
	int		Nx, Ny, Nz, Nt; /* # of bins */
	float	xs, ys, zs;		/* launch position */
    float   xdet, ydet, zdet;  //position of detector  // YL
	...
	fid = fopen(filename,"r");
		...
		fgets(buf, 32, fid);
		sscanf(buf, "%d", &Nphotons); // desired time duration of run [min]  //YL
		fgets(buf, 32, fid);
		...
		fgets(buf, 32,fid);
        sscanf(buf, "%f", &theta);  // theta     //YL
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &alpha);  // YL    
		fgets(buf, 32,fid);			
		sscanf(buf, "%f", &waist);  // waist
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &width);  // width   //YL 	
	...
	fclose(fid);  
        ...
        detYL = 1;  // 0: all photons; 1: only photons at Det
        rdet  = 0.02;
        xdet  = 0;
        ydet  = 0;
        zdet  = 0.5;
        
        detflag = 0;    // 0:photon doesn't arrive at detector, 1: photon arrive at detector
        Nphotons_Det = 0; 
        row = 0;
	...
	if (launchflag==1) {
        ...
        printf("theta = %0.4f [grad]\n",theta*180/PI); //YL
		printf("alpha = %0.4f grad\n",alpha*180/PI);  //YL
		printf("waist  = %0.4f [cm]\n",waist);
        printf("width  = %0.4f [cm]\n",width);   //YL
	}
    ...
    // YL DETECTOR
	float	*R=NULL;
	float   **position;  // save positions of each photon   YL DETECTOR
	//char    filename_format[] = "detected_photon_%d.txt";
	//char    filename_det[sizeof(filename_format)];  
	int 	type;
	NN = Nx*Ny*Nz;
    ...
    // YL DETECTOR
	Fdet  = (float *)malloc(NN*sizeof(float));	/* relative fluence rate [W/cm^2/W.delivered] */
	position =(float **) malloc(sizeof(float *)*STEP);   // position of each photon  YL DETECTOR
	for(i=0;i<STEP;i++) {position[i] = (float *)malloc(sizeof(float)*3);}
	...
	if(detYL==1)
	{printf("******* Begin Monte Carlo: Simulation of photons between source and detector *******\n");
	printf("%s\n",myname);
    printf("requesting %d photons\n",Nphotons);
	}
	else 
	{printf("******** Begin Monte Carlo: Simulation of all photons **********\n");
    printf("%s\n",myname);
    printf("requesting %d photons\n",Nphotons);}   //YL
	
	Nphotons = 1000000; // will be updated to achieve desired run time, time_min.
	i_photon = 0;
	CNT = 0;

	for(i_photon=1;i_photon<=Nphotons;i_photon++)    //YL
	{
		/* Print out progress for user if mcflag < 3 */   //YL
	    temp = (double)i_photon;
	    if ( (mcflag < 3) & (temp >= 100)) 
		{
	    	if (temp<1000) {
		...
		*/
		...
		{ // use mcflag
			...
			{ 
				...
				//r		= (0.02/7.389)*sqrt(-log(rnd)); // radius of beam at launch point  //YL
				//while ((rnd = RandomGen(1,0,NULL)) <= 0.0); // avoids rnd = 0
				//phi		= rnd*2.0*PI;
				//  YL
			    //x	= -sqrt(r*r*cos(phi)*cos(phi)+radius*radius)*cos(alpha+atan(r*cos(phi)/radius)) ;
			    //y	= ys + r*sin(phi);
			    //z	= -radius*sin(alpha+atan(r*cos(phi)/radius))+1.5;
				x = xs ;
				y = ys ; 
				z = zs;
			    position[0][0]=x;
				...
				while ((rnd = RandomGen(1,0,NULL)) <= 0.0); // avoids rnd = 0
				r		= 0.02*sqrt(rnd); // radius of beam at focus  //YL
				while ((rnd = RandomGen(1,0,NULL)) <= 0.0); // avoids rnd = 0
				phi		= rnd*2.0*PI;
				// YL
				xfocus = 0 ; 
		...
		do {
			...
			do{  // while sleft>0   
				...
				{  
                    ...
                    if(detYL==1)
                    {  
                        // to test, if the position of photon within the circle of detector
                        circle = (x - xdet)*(x - xdet) + (y - ydet)*(y - ydet);
                        height = fabs(z - zdet);
                        if (circle <=(rdet*rdet)&&(height<=0.005))
                        {
						    detflag = 1;  // photon arrive at detector
					        photon_status = DEAD;
					    }
                    }
        ...
        {  
			...
			for (int jj = 1; jj <= NN; jj++){
		        F[jj] += Fdet[jj]; // YL for detector  				
		    }
			Nphotons_Det++;
			printf("xdet = %f [cm]; ",xdet);
	    	printf("ydet = %f [cm]; ",ydet);
	    	printf("zdet = %f [cm]; \n",zdet);
	    	printf("radiusDet = %f [cm]; ",rdet);
	    	printf(" \t height = %f [cm]; \n",height);
            printf("*************************************************************\n");

     

			   //save position file   YL  DETECTOR
		 
			   fid = fopen("detected.txt","a+");    // appended for each detected photon
			   for(i=0;i<=CNT;i++)
		       {  
               ...
               // save row file to plot each detected photon
			   FILE *f1 = fopen("row.txt","a+");
			   row += CNT+1;
			   fprintf(f1,"%d\n",row);
               fclose(f1);
	...
	if(detYL==1)
	{
	   effectivity = Nphotons_Det/Nphotons*100;
	   dist = sqrt((xdet-xs)*(xdet-xs)+(ydet-ys)*(ydet-ys)+(zdet-zs)*(zdet-zs));
	   printf("Distance between source and detector = %f [cm]\n",dist);
	   printf("Number of photons that reach the detector = %d\n", Nphotons_Det);
	   printf("Effectivity = %f[%%]\n",effectivity);
	}
    ...
    fclose(fid);
	if( detYL==1)
	printf("detected.txt saved!\n");
    printf("row.txt saved!\n");
...
//    printf("%s is done.\n",myname);
    ...
    free(R);
	free(Fdet);
	for(i = 0;i<STEP;i++) {free(position[i]);}
	free(position);
    return 0;
} /* end of main */
